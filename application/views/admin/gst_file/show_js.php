            ...
            success: function (response) {
                if (response.Responsecode == 200 && response.Data != null) {
                    ...
                    for (var i = 0; i < count; i++) {
                        pfWithdrawal.set(response.Data[i].gid, response.Data[i]);
                    }
...
//$('.updateDiv').hide();
    ...
    function changeStatus(id) {
        ...
        $('#id').val(id);
         if(temp.returntype == 'yes'){
         $('#digital_amount').val(servicecharges.get('8'));
         }else{
         $('#digital_amount').val(servicecharges.get('9'));
         }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < count; i++) {
                    docTable += ` <tr>
      <td>
          <a href="` + (url + 'documents/gst_file/' + response[i].docid + '.' + response[i].extension) + `" class="stretched-link" download>` + response[i].doctype + `</a>
      </td>
    </tr>`;
                }
                }else{
...
//                console.log(response);
    ...
    function returnStatus(id,st) {
        $.ajax({
            ...
            url: url + 'get_gst_return_remarks',
            type: 'post',
            data: {rowid: id},
            dataType: 'json',
            success: function (response) {
