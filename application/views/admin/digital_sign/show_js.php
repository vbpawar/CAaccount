        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < count; i++) {
                    docTable += ` <tr>
      <td>
          <a href="` + (url + 'documents/digital/' + response[i].docid + '.' + response[i].extension) + `" class="stretched-link" download>` + response[i].doctype + `</a>
      </td>
    </tr>`;
                }
...
//                console.log(response);
    ...
    function returnStatus(id,st) {
        $.ajax({
            ...
            var did=id.toString();
            var product = digitalSign.get(did);
                const count = response.length;
