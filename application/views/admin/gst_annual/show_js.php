...
?>
    ...
    function changeStatus(id) {
        ...
        $('#id').val(id);
         $('#digital_amount').val(servicecharges.get('7'));
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < count; i++) {
                    docTable += ` <tr>
      <td>
          <a href="` + (url + 'documents/gst_annual/' + response[i].docid + '.' + response[i].extension) + `" class="stretched-link" download>` + response[i].doctype + `</a>
      </td>
    </tr>`;
                }
                }else{
