            ...
            success: function (response) {
                ...
                if (response.Responsecode == 200 && response.Data != null) {
                    ...
                    for (var i = 0; i < count; i++) {
                        pfWithdrawal.set(response.Data[i].companyid, response.Data[i]);
                    }
...
//        $('#includeBox').load('services/certificate/update'); 
            $.ajax({
                type: 'get',
                url: url + 'companyregistration/update',
                dataType: 'html',
...
//$('.updateDiv').hide();
    ...
    function changeStatus(id) {
        ...
        $('#cid').val(id);
        $('#digital_amount').val(servicecharges.get('16'));
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < count; i++) {
                    docTable += ` <tr>
      <td>
          <a href="` + (url + 'documents/company_docs/' + response[i].docid + '.' + response[i].extension) + `" class="stretched-link" download>` + response[i].doctype + `</a>
      </td>
    </tr>`;
                }
